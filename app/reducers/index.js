...
const directories = [homedir + '/.protonwallet', homedir + '/.protonwallet/logs', homedir + '/.protonwallet/wallets'];
window.directories = directories;
const [programDirectory, logDirectory, walletDirectory] = directories;
log.debug('Checking if program directories are present...');
directories.forEach(function(dir) {
  if (!fs.existsSync(dir)){
      log.debug('No directories detected, initial startup detected. Running setup...')
      fs.mkdirSync(dir);
  }
});
...
window.session.wallet.saveWalletToFile(walletDirectory + '/proton.wallet', 'hunter2');
